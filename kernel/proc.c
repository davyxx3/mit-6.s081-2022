...
// add mappings to kernel page table
void kpagetablemap(pagetable_t page_table, uint64 va, uint64 pa, uint64 sz, int perm)
{
  if (mappages(page_table, va, sz, pa, perm) != 0)
    panic("kvmmap");
}

// initalize the kernel page table for every process
pagetable_t kpagetable()
{
  pagetable_t ret = (pagetable_t)kalloc();
  memset(ret, 0, PGSIZE);

  // uart registers
  kpagetablemap(ret, UART0, UART0, PGSIZE, PTE_R | PTE_W);
  ...
  kpagetablemap(ret, KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  kpagetablemap(ret, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  kpagetablemap(ret, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
...
found:
  ...
  // initialize the kernel page table(create mappings of kernel stack)
  pte_t *pte = walk(kernel_pagetable, p->kstack, 0);
  uint64 pa = PTE2PA(*pte);
  kpagetablemap(p->k_pagetable, p->kstack, pa, PGSIZE, PTE_R | PTE_W);
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        // switch to kernel page table in the process
